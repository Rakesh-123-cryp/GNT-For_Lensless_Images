...
def create_eval_dataset(args):
    print("Evaluation dataset: {}".format(args.eval_dataset))
    mode = "eval"
    
    train_dataset = None
    train_sampler = None
    
    if "+" not in args.eval_dataset:
        train_dataset = dataset_dict[args.eval_dataset](args, mode, scenes=args.eval_scenes)
        train_sampler = (
            torch.utils.data.distributed.DistributedSampler(train_dataset)
            if args.distributed
            else None
        ...
        print("Please provide one directory")
