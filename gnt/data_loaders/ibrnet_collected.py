...
class IBRNetCollectedDataset(Dataset):
    def __init__(self, args, mode, random_crop=True, **kwargs):
        self.folder_path1 = os.path.join(args.rootdir, "data/new_blur/ibrnet_collected_1/")
        self.folder_path2 = os.path.join(args.rootdir, "data/new_blur/ibrnet_collected_2/")
        self.folder_path3 = os.path.join(args.rootdir, "data/new_blur/ibrnet_collected_more/")

        refined_dir = os.path.join(args.rootdir,'data/refined_data/')
        self.rectify_inplane_rotation = args.rectify_inplane_rotation
        self.mode = mode  # train / test / validation
        self.num_source_views = args.num_source_views
        self.random_crop = random_crop

        all_scenes = glob.glob(self.folder_path1 + "*") + glob.glob(self.folder_path2 + "*") + glob.glob(self.folder_path3 + "*")
        ...
        for i, scene in enumerate(all_scenes):
            if "ibrnet_collected_2" in scene or "ibrnet_collected_more" in scene:
                factor = 8
            else:
                factor = 2
            ...
            refined_path = [
                            os.path.join(refined_dir, '/'.join(rgb_file.split('/')[-4:]))
            ...
            self.num_render.append(np.arange(len(self.render_rgb_files),len(self.render_rgb_files)+num_render).tolist())
    ...
    def __getitem__(self, idx):
        ...
        rgb_file = os.path.join("data/refined_data","/".join(rgb_file.split("/")[3:]))
        ...
        train_rgb_files = self.train_rgb_files[train_set_id] #Scene Files
        render_rgb_files = np.array(self.render_rgb_files)[self.num_render[train_set_id]].tolist()
        ...
        if self.mode == "train":
            id_render = train_rgb_files.index(os.path.join("./data/new_blur", "/".join(rgb_file.split("/")[2:])))
        ...
        for id in nearest_pose_ids:
            ...
            # print(os.path.join("./data/blurred", "/".join(rgb_file.split("/")[2:])) == train_rgb_files[id])
