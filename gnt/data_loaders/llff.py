...
class LLFFDataset(Dataset):
    def __init__(self, args, mode, **kwargs):
        base_dir = os.path.join(args.rootdir, "data/new_blur/real_iconic_noface/")
        ...
        for i, scene in enumerate(scenes):
            ...
            self.train_rgb_files.append(np.array(rgb_files)[i_train].tolist()) #list of file paths for images
            num_render = len(i_render)
            self.num_render.append(list(np.arange(len(self.render_rgb_files),len(self.render_rgb_files)+num_render)))
            self.render_rgb_files.extend(np.array(rgb_files)[i_render].tolist()) #list of file paths for images
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            id_render = train_rgb_files.index(os.path.join("./data/new_blur", "/".join(rgb_file.split("/")[3:])))
            # print("id_render", train_rgb_files[id_render], rgb_file)
