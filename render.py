...
def log_view(
    ...
    img = np.asarray(rgb_coarse).astype('float32')  # Convert to float32
    img = (img * 255).astype(np.uint8)       # Scale to [0, 255] range
    img_pil = Image.fromarray(img) #
    imageio.imwrite(filename, img_pil)#
    ...
    if depth_coarse is not None:
        ...
        img = np.asarray(depth_coarse).astype('float32')  # Convert to float32
        img = (img * 255).astype(np.uint8)       # Scale to [0, 255] range
        img_pil = Image.fromarray(img) #
        imageio.imwrite(filename, img_pil)#
    ...
    if rgb_fine is not None:
        ...
        img = np.asarray(rgb_fine).astype('float32')  # Convert to float32
        img = (img * 255).astype(np.uint8)       # Scale to [0, 255] range
        img_pil = Image.fromarray(img) #
        imageio.imwrite(filename, img_pil)#
    ...
    if depth_fine is not None:
        ...
        img = np.asarray(depth_fine).astype('float32')  # Convert to float32
        img = (img * 255).astype(np.uint8)       # Scale to [0, 255] range
        img_pil = Image.fromarray(img) #
        imageio.imwrite(filename, img_pil)#
