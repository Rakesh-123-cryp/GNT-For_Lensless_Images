...
from gnt.transformer_network import Weiner
...
def train(args):
    ...
    # Freeze network
    for param in model.net_coarse.parameters():
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        #     print("DATA SHAPE: ", train_data['rgb_path'], train_data['src_rgbs'].shape)
        ...
        for train_data in train_loader:
            ...
            # Trainable weiner filter
            # save = np.hstack(train_data['src_rgbs'].squeeze(0)[:2].detach().cpu().numpy())
            # save = cv2.cvtColor(save, cv2.COLOR_BGR2RGB)
            # cv2.imwrite("trainable_weiner{}.png".format(random.random()), save*255)
            train_data['src_rgbs'] = model.trainable_inverse(train_data["src_rgbs"].to("cuda")).unsqueeze(0)
            # save = np.hstack(train_data['src_rgbs'].squeeze(0)[:2].detach().cpu().numpy())
            # save = cv2.cvtColor(save, cv2.COLOR_BGR2RGB)
            # cv2.imwrite("trainable_weiner{}.png".format(random.random()), save*255)
            ...
            #print("shape - {}".format(ret["outputs_coarse"]["rgb"].shape))
            # compute loss
            ...
            if args.local_rank == 0:
                ...
                if global_step % args.i_img == 0:
                    ...
                    # val_data["src_rgbs"] = model.trainable_inverse(val_data["src_rgbs"].to("cuda")).unsqueeze(0)
    ...
    print("Saving checkpoints at {} to {}...".format(global_step, out_folder))
    fpath = os.path.join(out_folder, "model_final.pth")
    model.save_model(fpath)
...
def log_view(
    ...
    average_im = ray_sampler.src_rgbs.cpu()[0][0]#.mean(dim=(0, 1))
    ...
    cv2.imwrite(filename, cv2.cvtColor(rgb_im*255, cv2.COLOR_BGR2RGB))
    if depth_im is not None:
        ...
        cv2.imwrite(filename, cv2.cvtColor(depth_im*255, cv2.COLOR_BGR2RGB))
